...
export async function register(
  ...
  const user = await prisma.user.create({
    data: { id: uuidv4(), name, email, passwordHash, freeQuota: 20 },
  });
...
export async function login(
  ...
  return {
    token,
    user: { id: user.id, email: user.email, name: user.name, isAdmin: user.isAdmin, dashscopeKey: user.dashscopeKey, freeQuota: user.freeQuota },
  };
}
...
export async function getMe(userId: string): Promise<object> {
  ...
  return { id: user.id, email: user.email, name: user.name, isAdmin: user.isAdmin, dashscopeKey: user.dashscopeKey, freeQuota: user.freeQuota };
}
