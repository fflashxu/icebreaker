...
async function generateSingleEmail(
  openai: OpenAI,
  model: string,
  req: Omit<GenerateRequest, 'apiKey' | 'baseUrl' | 'model' | 'provider' | 'count'>,
  angleHint: string
...
export async function generateEmails(req: GenerateRequest): Promise<GeneratedEmail[]> {
  const { count, apiKey, baseUrl, model, ...rest } = req;
  const openai = createClient(apiKey, baseUrl);

  const tasks = Array.from({ length: count }, (_, i) =>
    generateSingleEmail(openai, model, rest, ANGLE_HINTS[i] ?? ANGLE_HINTS[0])
  );
